...
const Quiz: React.FC = () => {
    ...
    return (
        <div className={`min-h-screen pt-20 flex flex-col`}>
            <div className="container flex justify-center items-center bg-white py-32  rounded-lg shadow-lg min-h-[800px] mt-16"
                style={{
                ...
                {quizFinished ? (
                    <div className="flex flex-col items-center w-full">
                        <div className='flex flex-col items-center text-center bg-white py-10  rounded-lg shadow-lg lg:w-3/4 w-full'>
                            <p className="absolute -translate-y-16 bg-green-600 text-white text-2xl font-bold lg:w-1/4 w-1/2 rounded-md py-2">Kuis selesai!</p>
                            <p className="text-2xl font-bold bg-white p-5  rounded-lg">Anda Benar {correctAnswers} dari {totalQuestions} <br /> Skor Anda: <br /></p>
                            
                            <div className='text-4xl font-bold text-kuning p-5  rounded-full bg-biru mb-10'>{Math.round((correctAnswers / totalQuestions) * maxScore)}</div> 
                                <button onClick={toggleExplanation} className="items-center lg:w-3/4 w-3/4 flex h-14 bg-white border text-primary border-black rounded-full text-lg hover:bg-accent mb-4">
                                ...
                                <button onClick={restartQuiz} className="relative lg:w-3/4 mt-5 w-3/4 flex h-14 pl-5 bg-accent text-primary py-3 border border-black rounded-full text-lg hover:text-white hover:bg-accent">
                    ...
                    <div className="flex flex-col items-center w-full">
                        <div className='flex flex-col items-center text-center bg-white py-10  rounded-lg shadow-lg lg:w-3/4 w-full'>
                            <p className="absolute -translate-y-16 text-primary bg-accent text-lg font-bold lg:w-1/4 w-1/2 rounded-md py-2">Soal {currentQuestion + 1} dari {totalQuestions}</p>
                            <p className="mt-4 bg-white py-8  rounded-lg lg:text-xl px-2">{quizData[currentQuestion].question}</p>
